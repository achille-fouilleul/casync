...
CASYNC_PUBLIC CaIndex *ca_index_new_read(void); /* only (cooked) reading */
...
CASYNC_PUBLIC CaIndex *ca_index_unref(CaIndex *i);
static inline void ca_index_unrefp(CaIndex **i) {
        ca_index_unref(*i);
}

int ca_index_set_fd(CaIndex *i, int fd);
CASYNC_PUBLIC int ca_index_set_path(CaIndex *i, const char *path);

int ca_index_set_make_mode(CaIndex *i, mode_t m);

CASYNC_PUBLIC int ca_index_open(CaIndex *i);
...
CASYNC_PUBLIC int ca_index_read_chunk(CaIndex *i, CaChunkID *id, uint64_t *ret_offset_end, uint64_t *ret_size);

CASYNC_PUBLIC int ca_index_set_position(CaIndex *i, uint64_t position);
CASYNC_PUBLIC int ca_index_get_position(CaIndex *i, uint64_t *ret);
CASYNC_PUBLIC int ca_index_get_available_chunks(CaIndex *i, uint64_t *ret);
...
CASYNC_PUBLIC int ca_index_get_total_chunks(CaIndex *i, uint64_t *ret);
